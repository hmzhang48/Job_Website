...
watch([userState, hrState, guideState], async () => {
  await stateCheck()
...
const start = (buttonState: number) =>
  void (buttonState ? router.push({ name: 'index' }) : router.push({ name: 'login' }))
const updateInfo = () =>
  route.name === 'infoBox' ? router.go(0) : void router.push({ name: 'infoBox' })
...
const stateCheck = async () => {
  if (userState.value) {
    if (guideState.value) {
      void (hrState.value ? router.push({ name: 'guideHR' }) : router.push({ name: 'guideUser' }))
    }
    else {
      if (hrState.value) {
        void router.push({ name: 'hrPage' })
        ...
        void router.push({ name: 'userPage' })
...
provide(updateKey, (key: string, value: string) => info.value[key] = value)
provide(resetKey, () => {
