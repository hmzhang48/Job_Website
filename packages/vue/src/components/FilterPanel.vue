...
<template>
  <article>
    ...
    <fieldset>
      ...
      <div class=".grid">
        <label for="full-time">
          <input v-model=" type " type="radio" name="jobtype" id="full-time" value="full-time" />
          全职
        </label>
        <label for="part-time">
          <input v-model=" type " type="radio" name="jobtype" id="part-time" value="part-time" />
          兼职
        </label>
      </div>
    </fieldset>
    <label for="salary">{{ `期望薪资(${ salaryType })` }}</label>
    <input v-model.lazy=" salary " type="range" min="0" max="100" id="salary" name="salary" />
    <small>{{ salary }}</small>
    <fieldset>
      <legend>工作地点</legend>
      <div class="grid">
        <label for="local">
          <input v-model=" location " type="radio" name="location" id="local" value="local" />
          本地
        </label>
        <label for="out">
          <input v-model=" location " type="radio" name="location" id="out" value="out" />
          外地
        </label>
      </div>
    </fieldset>
