<script setup lang="ts">
  ...
  const checkPhone = () => {
    resetCode()
    invalidPhone.value = phoneRegex.test(newPhone.value) ? false : true
  }
  ...
  const checkCode = async () => {
    if (validCode !== "" && code.value === validCode) {
      ...
      let result = await (hrState.value
        ? patchHRInfo(newPhone.value)
        : patchUserInfo({ phone: newPhone.value }))
      if (result) {
        p.value = false
...
<template>
  <article>
    <div class="row">
      <p class="item"><strong>姓名</strong></p>
      <div class="content">
        <span>{{ info?.["name"] }}</span>
    ...
    <div v-if="hrState">
      <div class="row">
        <p class="item"><strong>公司名称</strong></p>
        <div class="content">
    ...
    <div v-if="hrState">
      <div class="row">
        <p class="item"><strong>统一社会信用代码</strong></p>
        <div class="content">
    ...
    <div v-if="!hrState">
      <div
        class="row"
        v-show="!l">
        <p class="item"><strong>居住地</strong></p>
        <div class="content">
          <span>{{ location }}</span>
          <mark @click.prevent="() => (l = true)">修改</mark>
        </div>
      </div>
      <div v-show="l">
        <fieldset role="group">
    ...
    <div>
      <div
        class="row"
        v-show="!p">
        <p class="item"><strong>手机号码</strong></p>
        <div class="content">
          <span>{{ info?.["phone"] }}</span>
          <mark @click.prevent="() => (p = true)">修改</mark>
        </div>
      </div>
      <div v-show="p">
        <input
