...
<template>
  <article>
    <div class="grid">
      <label for="datetime">
        面试时间
        <input v-model.lazy=" date " type="date" id="date" :aria-invalid=" invalid.date " required />
        <input v-model.lazy=" time " type="time" id="time" :aria-invalid=" invalid.time " />
        <small v-show=" invalid.date || invalid.time ">请输入面试时间</small>
      </label>
      <label for="location">
        面试地点
        <input v-model.lazy=" location " type="text" id="location" placeholder="请输入面试地点" :aria-invalid=" invalid.location "
          required />
