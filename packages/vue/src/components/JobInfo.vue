...
const checkPosition = () => {
  if ( position.value === "" ) {
    invalid.value.position = true
  } else {
    invalid.value.position = false
  }
}
let overview = ref( "" )
const checkOverview = () => {
  if ( overview.value === "" ) {
    invalid.value.overview = true
  } else {
    invalid.value.overview = false
  }
}
let type = ref( "" )
...
const checkSalary = () => {
  if (
    ...
    invalid.value.salary = true
  } else {
    invalid.value.salary = false
  }
}
...
onMounted( () => {
  ...
  if ( props.no && props.job ) {
    ...
    if ( l ) {
      for ( let i = 0; i < l; i++ ) {
        let option = provinceSelect.value?.options.item( i )
    ...
    if ( l ) {
      for ( let i = 0; i < l; i++ ) {
        let option = citySelect.value?.options.item( i )
    ...
    if ( l ) {
      for ( let i = 0; i < l; i++ ) {
        let option = areaSelect.value?.options.item( i )
...
<template>
  <article>
    ...
    <input v-model=" position " :[invalidKey]=" invalid.position " @change=" checkPosition " id="position" type="text"
      placeholder="职位名称" required />
    <p><small v-show=" invalid.position ">请填写职位名称</small></p>
    <label for="overview">职位介绍</label>
    <textarea v-model=" overview " :[invalidKey]=" invalid.overview " @change=" checkOverview " id="overview" rows="3"
      placeholder="职位介绍"></textarea>
    <p><small v-show=" invalid.overview ">请填写职位介绍</small></p>
    <fieldset>
      <legend>职位性质</legend>
      <div class="grid">
        <label for="full-time">
          <input v-model=" type " :[invalidKey]=" invalid.type " @change=" checkType " type="radio" name="type" id="full-time"
            value="full-time" />
          全职
        </label>
        <label for="part-time">
          <input v-model=" type " :[invalidKey]=" invalid.type " @change=" checkType " type="radio" name="type" id="part-time"
            value="part-time" />
          兼职
        </label>
      </div>
    ...
    <fieldset>
      ...
      <div class="grid">
        <label for="minSalary">
          <input v-model=" minSalary " :[invalidKey]=" invalid.salary " @change=" checkSalary " id="minSalary" type="number"
            placeholder="最低薪资" />
        </label>
        <label for="maxSalary">
          <input v-model=" maxSalary " :[invalidKey]=" invalid.salary " @change=" checkSalary " id="maxSalary" type="number"
            placeholder="最高薪资" />
        </label>
      </div>
    ...
    <fieldset>
      <legend>工作地点</legend>
      <div class="grid">
        <select id="province" :[invalidKey]=" invalid.location " ref="provinceSelect" @change=" addCity ">
          <option value="province" selected>省(直辖市)</option>
        </select>
        <select id="city" :[invalidKey]=" invalid.location " ref="citySelect" @change=" addArea ">
          <option value="city" selected>市</option>
        </select>
        <select v-model.lazy=" location " id="area" :[invalidKey]=" invalid.location " ref="areaSelect"
