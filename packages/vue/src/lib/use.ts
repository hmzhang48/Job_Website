...
export const useCanvas = async (
  canvas: Ref<HTMLCanvasElement | undefined>,
  initial: HTMLImageElement
) => {
...
export const useObserver =
  ( element: Ref<HTMLElement | undefined>, f: () => Promise<void>, end: Ref<boolean> ) => {
    let radio = 0
    const observer = new IntersectionObserver(
      async ( entries ) => {
        for ( const entry of entries ) {
          if (
            entry.intersectionRatio > radio && entry.intersectionRatio === 1 && !end.value
          ) {
            await f()
          }
          radio = entry.intersectionRatio
        }
      }, { threshold: [ 0, 1 ] }
    )
    onMounted( () => {
