...
export const corpRelations = relations( corpInfo, ( { many } ) => ( {
  hrList: many( hrInfo )
} ) )
export const hrRelations = relations( hrInfo, ( { one } ) => ( {
  corpInfo: one( corpInfo, {
    fields: [ hrInfo.corpID ],
    references: [ corpInfo.corpID ]
  } )
} ) )
export const jobRelations = relations( jobInfo, ( { one } ) => ( {
  corpInfo: one( corpInfo, {
