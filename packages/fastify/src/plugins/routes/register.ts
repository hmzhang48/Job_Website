...
const register: FastifyPluginCallback = fp((f, _, done) => {
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      void reply.send({ result: result ? code : '' })
    },
  )
  server.get(
    '/phone-validate',
